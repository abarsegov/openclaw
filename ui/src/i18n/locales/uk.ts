...
export const uk: TranslationMap = {
  ...
  updates: {
    ...
    outcomeUnknown:
      "Запит на оновлення міг бути прийнятий, але Gateway не повідомив остаточний результат після повторного підключення. Виконайте `openclaw update status` перед повторною спробою.",
    failureReasons: {
  ...
  configForm: {
    ...
    invalidJson: "Введіть дійсний JSON, перш ніж залишити це поле.",
    invalidString: "Введіть значення, яке відповідає обмеженням цього налаштування.",
    invalidNumber: "Введіть значення в межах допустимого діапазону та кроку.",
    draftRejected: "Це налаштування не вдалося зберегти. Ваш чернетковий варіант досі тут.",
  ...
  modelSetup: {
    ...
    prepare: {
      ...
      llamaCppNotReady:
        "llama.cpp не створив придатної локальної моделі. Перегляньте результат налаштування, а потім повторіть спробу.",
    },
    manual: {
      title: "Підключення за допомогою ключа API або токена",
      provider: "Постачальник",
      selectProvider: "Виберіть постачальника",
      selectProviderHint: "Виберіть, звідки надходять ці облікові дані",
      ...
      verifyHint:
        "OpenClaw перевіряє реальну відповідь моделі, перш ніж позначити підключення готовим.",
      required: "Виберіть постачальника та введіть ключ API або токен.",
    },
    success: {
      title: "Ваш ШІ готовий",
      body: "OpenClaw отримав реальну відповідь від {modelRef}. Тепер ви можете розпочати спілкування.",
  ...
  custodian: {
    ...
    nudge: {
      ...
      channelSetupTitle: "Зв'язок з OpenClaw поза цим застосунком",
      channelSetupBody:
        "Веб-застосунок уже працює. Додавайте канал, лише якщо хочете писати OpenClaw через інший сервіс.",
  ...
  mcpServers: {
    ...
    sessionEnableFailed:
      "Сервер збережено глобально вимкненим, але не вдалося увімкнути його для цієї сесії: {error}",
  ...
  dreaming: {
    ...
    phrases: {
      ...
      defragmentingMemoryLane: "дефрагментація доріжки пам'яті…",
  ...
  modelProviders: {
    ...
    readiness: {
      ...
      signedInNoModels:
        "Ви ввійшли, але цей обліковий запис не надає придатних моделей. Виберіть іншого провайдера або обліковий запис, щоб продовжити.",
      notConfigured: "Виберіть провайдера й перевірте модель, яку використовуватиме OpenClaw.",
      noModels: "Немає доступних моделей",
      modelRequired: "Потрібна модель",
      chooseProvider: "Вибрати іншого провайдера",
    },
    logout: {
      action: "Вийти",
